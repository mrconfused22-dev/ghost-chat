...
function BottomBtn({ item, active, onClick, unread }) {
  ...
  return (
    <button onClick={onClick}
      ...
      <svg width="20" height="20" viewBox="0 0 24 24" fill={col} style={{ transition: 'fill 0.2s' }}>
        <path d={item.svg} />
      </svg>
...
function Dashboard({ user, onLogout }) {
  ...
  const settingsPanel = (
    <div style={{ flex: 1, overflow: 'auto', background: '#000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 'clamp(1rem, 5vw, 2rem)', position: 'relative' }}>
        ...
        <div style={{ border: '1px solid #1a1a1a', borderRadius: '8px', overflow: 'hidden', marginBottom: '1rem' }}>
          ...
          <div style={{ padding: '1rem', borderBottom: '1px solid #1a1a1a' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { navigator.clipboard.writeText(user.friendCode); alert('Copied!') }} style={{ background: '#fff', color: '#000', border: 'none', padding: '0.3rem 0.6rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 900, flexShrink: 0 }}>COPY</button>
            </div>
          </div>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', height: '100dvh', background: '#000', overflow: 'hidden' }}
      ...
      <div style={{ display: 'none', flexDirection: 'column', height: '100vh', height: '100dvh', background: '#000', overflow: 'hidden' }}
        ...
        <div style={{ height: '58px', background: '#050505', borderTop: '1px solid #111', display: 'flex', alignItems: 'stretch', flexShrink: 0, paddingBottom: 'env(safe-area-inset-bottom)' }}>
          ...
          <button
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill={showSettings ? '#fff' : '#444'} style={{ transition: 'fill 0.2s' }}>
              <path d={SETTINGS_SVG} />
            </svg>
